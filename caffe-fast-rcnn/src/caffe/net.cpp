...
// <Iasonas>
// get GPU memory usage  (adapt to your setting)
#define CMD "nvidia-smi |grep '0  GeForce' -A 2| grep 250 "
// generate debugging info
bool dbgn  = false;
bool dbgst = false;
// generate debugging info for memory usage
//bool dbgm = false;
// use cheap variant of forward pass during test-time
bool do_cheap = true;
char command[50];
...
//< Iasonas>
// freeze the updates for layers that do not receive ground truth at this
// training sample
template <typename Dtype>
void Net<Dtype>::ResetLearningRateAndWeightDecay(const bool* shun){
  params_lr_.clear();
  params_weight_decay_.clear();
  ParamSpec default_param_spec;
  for (int layer_id = 0; layer_id < layers_.size(); ++layer_id) {
    bool shun_it = shun[task_at_layer_[layer_id]];
    layers_[layer_id]->SetShun(shun_it);

    for (int j = 0; j < layers_[layer_id]->blobs().size(); ++j) {
      if (shun_it)
      {
        params_lr_.push_back(0.0);
        params_weight_decay_.push_back(0.0);
      }
      else
      {
        const ParamSpec* param_spec =
          (layers_[layer_id]->layer_param().param_size() > j) ?
          &layers_[layer_id]->layer_param().param(j) : &default_param_spec;
        params_lr_.push_back(param_spec->lr_mult());
        params_weight_decay_.push_back(param_spec->decay_mult());
      }
    }
...
// For graduated-DSN: inform each layer about the current update iteration
template <typename Dtype>
void Net<Dtype>::SetTime( Dtype time) {
  for (int i = 0; i < layers_.size(); ++i)
  {
    layers_[i]->SetTime(time);
  }
}
//</Iasonas>
...
Dtype Net<Dtype>::ForwardFromTo(int start, int end) {
  ...
  if (do_cheap)
  {
    for (int i=0;i<NTASKS; i++)
        shun[i] = false;
    const char* args[] = {TASKNAMES};
    vector<string> tasks(args, args + (NTASKS + TASKOFFSET));
    strcpy( command, CMD);

    for (int i = start; i <= end; ++i) {
      if (dbgn) {LOG(INFO) << "Forward i: "<<i <<" layer: "<< layer_names_[i];}
      layers_[i]->Forward(bottom_vecs_[i], top_vecs_[i]);
      if (dbgn)
        {
          for (int k = 0;k<bottom_vecs_[i].size();k++)
            LOG(INFO) << bottom_vecs_[i][k]->shape_string();
          }
    	// Iasonas code: remove unnecessary layers (as in ForwardBackwardCheap)
      if (layers_[i]->layer_param().deletebottom())
      {
          if (dbgn)
          {LOG(INFO)<<"deleting bottom blobs of layer: "<<i << layer_names_[i];
           LOG(INFO)<<"would be: ";
          //if (dbgm) system(command);
          }
          for (int k = 0;k<bottom_vecs_[i].size();k++)
          {
              bottom_vecs_[i][k]->RemoveBoth();
              if (dbgn) LOG(INFO)<<"deleting bottom blob: "<< k << " of layer: "<<i << " =  "<<layer_names_[i];;
          }
      }
      //</Iasonas>
    }
